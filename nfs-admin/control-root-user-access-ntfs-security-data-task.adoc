...
summary: Sie können ONTAP so konfigurieren, dass NFS-Clients auf Daten im NTFS-Sicherheitsstil zugreifen können und NTFS-Clients auf Daten im NFS-Sicherheitsstil zugreifen können. Wenn Sie den NTFS-Sicherheitsstil auf einem NFS Datastore verwenden, müssen Sie entscheiden, wie der Zugriff durch den Root-Benutzer behandelt werden soll, und die Storage Virtual Machine entsprechend konfigurieren. 
---
