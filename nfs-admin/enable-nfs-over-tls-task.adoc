...
====
* Clients müssen RPC-with-TLS über TCP unterstützen, so konfiguriert sein, dass sie TLS beim Mounten anfordern, und der Zertifizierungsstelle vertrauen, die das NFS-Serverzertifikat signiert hat. Die Konfigurationsschritte auf Client-Seite sind hier nicht dokumentiert, da sie je nach Client variieren. Eine vollständige End-to-End-Anleitung für einen Linux-Client befindet sich unter link:https://docs.netapp.com/us-en/ontap-technical-reports/nfs-tls/nfs-tls-configure.html["NFS über TLS konfigurieren"].
