...
summary: Sie können die Dauer konfigurieren, für die ONTAP Anmeldeinformationen für NFS-Benutzer im internen Cache speichert, indem Sie den NFS-Server der Storage Virtual Machine anpassen. Dadurch können Sie bestimmte Probleme im Zusammenhang mit hoher Auslastung der Nameserver oder Änderungen der Anmeldeinformationen, die den NFS-Benutzerzugriff beeinflussen, beheben. 
---
