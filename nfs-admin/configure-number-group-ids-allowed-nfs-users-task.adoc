...
NOTE: Um die AUTH_SYS-Authentifizierungsvoraussetzungen für die Aktivierung erweiterter Gruppen zu verstehen(`-auth-sys-extended-groups` ), die Gruppen-IDs über das Standardmaximum von 16 hinaus verwenden, finden Sie imlink:https://kb.netapp.com/on-prem/ontap/da/NAS/NAS-KBs/What_are_the_prerequisites_for_enabling_auth_sys_extended_groups#["NetApp Knowledge Base: Was sind die Voraussetzungen für die Aktivierung von auth-sys-extended-groups?"^]
...
* link:https://kb.netapp.com/on-prem/ontap/da/NAS/NAS-KBs/How_does_AUTH_SYS_Extended_Groups_change_NFS_authentication["NetApp Knowledge Base: AUTH_SYS Extended Groups-Änderungen für die NFS-Authentifizierung für ONTAP 9"^]
