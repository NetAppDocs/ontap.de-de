...
.Bevor Sie beginnen
Wenn Sie die FlexCache Write-Back-Funktion aktiviert haben, müssen Sie sie deaktivieren, bevor Sie ein FlexCache Volume löschen können. Siehe link:../flexcache-writeback/flexcache-writeback-enable-task.html#disable-write-back-on-a-flexcache-volume["Deaktivieren Sie Write-Back auf einem FlexCache-Volume"].
