...
* Ab ONTAP 9.18.1 unterstützt FlexCache das Erstellen von Cache-Volumes für Ursprungsvolumes mit SVMs, die zu einer SVM-DR-Beziehung gehören.
+
Wenn Sie ONTAP 9.18.1 oder höher verwenden, muss ein Speicheradministrator die Cache-SVMs sowohl mit den primären als auch den sekundären Ursprungs-SVMs, die Teil einer SVM-DR-Beziehung sind, verbinden, bevor Cache-Volumes von Ursprungs-Volumes erstellt werden, die Teil einer SVM-DR-Beziehung sind.
