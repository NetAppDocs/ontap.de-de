...
Unterstützt ab ONTAP 9.5. Die primäre SVM einer SVM-DR-Beziehung kann das Ursprungsvolume haben; wenn Sie jedoch eine ONTAP Version vor ONTAP 9.18.1 verwenden, muss die FlexCache Beziehung mit einem neuen Ursprungsvolume neu erstellt werden, wenn die SVM-DR-Beziehung unterbrochen wird.

Ab ONTAP 9.18.1 werden die Caches bei einem Failover des Ursprungs-SVM automatisch auf den Ursprung am DR-Standort umgeschaltet. Manuelle Wiederherstellungsschritte entfallen.

xref:create-volume-task.adoc[Erfahren Sie mehr über die Erstellung von FlexCache -Volumes.]
 a| 
Nein
