...
+
.. Einen Snapshot auf dem Quellvolume in der SnapMirror asynchronen Beziehung erstellen:
+
`volume snapshot create -snapshot snapshot_name -vserver _SVM_name_ -volume _volume_name_`

.. Die SnapMirror Beziehung wird aktualisiert, um den Basissnapshot auf das Ziel zu verschieben:
+
