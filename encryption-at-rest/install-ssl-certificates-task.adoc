...
NOTE: `admin_SVM` ist die Admin-SVM für den Cluster. In den folgenden Beispielen heißt die Admin-SVM  `cluster1`, daher erscheint  `cluster1` sowohl in der Eingabeaufforderung (`cluster1::>`) als auch als Wert für  `-vserver`.
