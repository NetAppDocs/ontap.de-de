...
.Über diese Aufgabe
Sie müssen die ONTAP Befehlszeilenschnittstelle (CLI) verwenden, um externe Schlüsselmanager für MetroCluster-Konfigurationen zu konfigurieren. Erfahren Sie mehr über link:configure-external-key-management-concept.html["Konfiguration der externen Schlüsselverwaltung von ONTAP über die Befehlszeile"].
