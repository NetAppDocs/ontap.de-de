...
* Im ONTAP Mediator wird das SCST-Paket verwendet, um iSCSI für die Kommunikation in MetroCluster IP-Konfigurationen bereitzustellen. Das SCST-Paket ist ein Kernelmodul, das während der Installation kompiliert wird. Ab ONTAP 9.19.1 und ONTAP Mediator 1.12 ist die Konfiguration eines ONTAP Mediator möglich, der HTTPS anstelle von iSCSI zur Überwachung einer MetroCluster IP-Konfiguration verwendet.
