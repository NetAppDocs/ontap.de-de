...
ONTAP Mediator benötigt SCST, um iSCSI bereitzustellen (siehe  http://scst.sourceforge.net/index.html[]). Dieses Paket ist ein Kernelmodul, das während der Installation speziell für den Kernel kompiliert wird. Alle Aktualisierungen des Kernels können erfordern, dass SCST neu installiert werden muss. Alternativ deinstallieren Sie ONTAP Mediator, installieren es erneut und konfigurieren dann die ONTAP-Beziehung neu.


NOTE: Jegliche Aktualisierungen des Server-OS-Kernels sollten mit einem Wartungsfenster in ONTAP abgestimmt werden.
