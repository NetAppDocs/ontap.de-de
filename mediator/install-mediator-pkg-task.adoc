...
[CAUTION]
====
* In MetroCluster IP-Konfigurationen kann eine einzelne ONTAP Mediator-Instanz, wenn der ONTAP Mediator mit dem HTTPS-Protokoll konfiguriert ist, nur eine MetroCluster IP-Konfiguration überwachen. Die gleichzeitige Überwachung mehrerer MetroCluster IP-Konfigurationen wird nicht unterstützt.
...
** Antworten Sie  `No`, wenn Sie ONTAP Mediator nur für SnapMirror active sync verwenden möchten oder wenn Sie ONTAP Mediator mit HTTPS konfigurieren, um eine einzelne MetroCluster IP-Konfiguration pro ONTAP Mediator-Instanz zu überwachen.
+
Alternativ können Sie die  `--https-only` Option im  `./ontap-mediator-1.12.0/ontap-mediator-1.12.0 -y` Befehl verwenden, um nur HTTPS anzugeben.
...
[NOTE]
====
Um die Signaturprüfung während der Installation zu überspringen, verwenden Sie diesen Befehl: `./ontap-mediator-1.12.0/ontap-mediator-1.12.0 -y --skip-code-signature-check`
...
NOTE: Nach der Installation werden im System die README-Dateien und der Speicherort der Schlüssel angezeigt.
...
====
. Starten Sie den Host neu und verwenden Sie den UEFI Boot Manager Ihres Geräts, um das neue MOK zu genehmigen.  Sie benötigen die mitgelieferte Passphrase für die `mokutil` Nutzen inlink:index.html#install-ontap-mediator-when-uefi-secure-boot-is-enabled["Installieren Sie ONTAP Mediator, wenn UEFI Secure Boot aktiviert ist"] Die
...
.Schritte
. Während des Build-Prozesses wird ein öffentliches/privates Schlüsselpaar generiert. `/opt/netapp/lib/ontap_mediator/ontap_mediator/SCST_mod_keys/` Verzeichnis, indem Sie den folgenden Befehl verwenden:
+
`[root@mediator-host ~]# ls /opt/netapp/lib/ontap_mediator/ontap_mediator/SCST_mod_keys/
README.module-signing  scst_module_key.der  scst_module_key.priv
[root@mediator-host ~]#`

. Starten Sie den Vorgang des Importierens des öffentlichen Schlüssels in das UEFI-Schlüsselrepository, indem Sie die folgenden Befehle ausführen:
+
...
. Die Software mokutil verlangt während des Importvorgangs ein temporäres Passwort für diesen Schlüssel.
. Überprüfen Sie, ob der Importvorgang mit dem `mokutil --list-new` und starten Sie das System anschließend neu.  Der Bootloader startet den EFI MOK Manager.
. Verwenden Sie die Menüs auf dem Bildschirm, um den SCST-Kernelmodulschlüssel zu aktivieren.  Nach dem Booten ausführen `systemctl status mediator-scst` Die  Sobald der Dienst startet, werden die SCST-Kernelmodule signiert.
