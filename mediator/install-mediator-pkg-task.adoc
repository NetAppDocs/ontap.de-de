...
summary: Um ONTAP Mediator zu installieren oder zu aktualisieren, müssen Sie das Installationspaket herunterladen und das Installationsprogramm auf dem Host ausführen. 
---
= Das ONTAP Mediator Installationspaket installieren
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
.Über diese Aufgabe
* Ab ONTAP 9.19.1 und ONTAP Mediator 1.12 besteht die Möglichkeit, ONTAP Mediator mit iSCSI und HTTPS-Unterstützung oder nur mit HTTPS-Unterstützung zu installieren. Wenn ONTAP Mediator nur mit HTTPS-Unterstützung installiert wird, wird das SCST-Paket nicht installiert.
...
* Während der Installation oder des Upgrades auf ONTAP Mediator 1.12 oder höher werden Sie vom Installationsprogramm aufgefordert, auf Folgendes zu antworten:
+
...
+
** Antworten Sie  `Yes`, wenn Sie planen, ONTAP Mediator mit iSCSI zu verwenden, um eine oder mehrere MetroCluster IP-Konfigurationen zu überwachen.
** Antworten `No` Sie, wenn geplant ist, ONTAP Mediator ausschließlich für SnapMirror active sync zu verwenden oder wenn ONTAP Mediator mit HTTPS konfiguriert werden soll, um eine oder mehrere MetroCluster IP-Konfigurationen zu überwachen.
+
Alternativ können Sie die  `--https-only` Option im  `./ontap-mediator-1.12.1/ontap-mediator-1.12.1` Befehl verwenden, um nur HTTPS anzugeben.



* Für Automatisierungsanwendungen kann der  `./ontap-mediator-1.12.1/ontap-mediator-1.12.1 -y`Befehl verwendet werden, um die Installationsabfragen zu überspringen.


.Schritte
. Führen Sie das Installationsprogramm aus und beantworten Sie die Eingabeaufforderungen wie erforderlich:
+
...
[NOTE]
====
Um die Signaturprüfung während der Installation zu überspringen, verwenden Sie diesen Befehl: `./ontap-mediator-1.12.1/ontap-mediator-1.12.1 --skip-code-signature-check`

====
+
Das Installationsprogramm erstellt die erforderlichen Konten und installiert die benötigten Pakete. Wenn der Mediator bereits installiert ist, erfolgt eine Aufforderung zum Upgrade.



.Beispiel ONTAP Mediator-Installation mit HTTPS- und iSCSI-Unterstützung (Konsolenausgabe)
[%collapsible]
====
[listing]
----
...
----
====
.Beispiel für ONTAP Mediator-Upgrade mit HTTPS- und iSCSI-Unterstützung (Konsolenausgabe)
[%collapsible]
====
[listing]
----
...
----
====
.Beispiel ONTAP Mediator-Installation mit Unterstützung nur für HTTPS (Konsolenausgabe)
[%collapsible]
====
[listing]
----
...
----
====
.Beispiel für ONTAP Mediator-Upgrade mit Unterstützung nur für HTTPS (Konsolenausgabe)
[%collapsible]
====
[listing]
----
...
== Den Sicherheitsschlüssel für UEFI Secure Boot registrieren

Ab ONTAP Mediator 1.4 ist der Secure Boot Mechanismus auf UEFI-Systemen aktiviert. Wenn Secure Boot aktiviert ist, sind nach der Installation zusätzliche Schritte zur Registrierung des Sicherheitsschlüssels erforderlich.
...
. Die benötigten Schlüssel befinden sich an folgender Stelle:
+
`/opt/netapp/lib/ontap_mediator/ontap_mediator/SCST_mod_keys`

+

NOTE: Nach der Installation gibt die Systemausgabe die README-Dateien und den Speicherort der Schlüssel an.

. Den öffentlichen Schlüssel zur MOK-Liste hinzufügen:
+
...
[NOTE]
====
Sie können den privaten Schlüssel am Standardspeicherort belassen oder ihn an einen sicheren Ort verschieben. Der öffentliche Schlüssel muss am aktuellen Speicherort verbleiben, damit der Boot Manager ihn verwenden kann. Weitere Informationen enthält die Datei README.module-signing:

`[root@hostname ~]# ls /opt/netapp/lib/ontap_mediator/ontap_mediator/SCST_mod_keys/
README.module-signing  scst_module_key.der  scst_module_key.priv`

====
. Starten Sie den Host neu und verwenden Sie den UEFI Boot Manager Ihres Geräts, um den neuen MOK zu genehmigen. Sie benötigen die für das  `mokutil` Dienstprogramm bereitgestellte Passphrase in link:index.html#install-ontap-mediator-when-uefi-secure-boot-is-enabled["ONTAP Mediator installieren, wenn UEFI Secure Boot aktiviert ist"].
...
Nach der Installation von ONTAP Mediator, falls der systemctl status `mediator-scst` als fehlgeschlagen (inaktiv) angezeigt wird, sind die folgenden Schritte zum Signieren des SCST-Kernelmoduls erforderlich.


NOTE: Wenn Sie ONTAP Mediator 1.12 oder höher mit Unterstützung nur für HTTPS installiert haben, wird das SCST-Paket nicht installiert und Sie können diese Aufgabe überspringen.

.Schritte
. Während des Build-Prozesses wird im  `/opt/netapp/lib/ontap_mediator/ontap_mediator/SCST_mod_keys/` Verzeichnis mithilfe des folgenden Befehls ein öffentliches/privates Schlüsselpaar generiert:
+
`[root@mediator-host ~]# ls /opt/netapp/lib/ontap_mediator/ontap_mediator/SCST_mod_keys/
README.module-signing  scst_module_key.der  scst_module_key.priv
[root@mediator-host ~]#`

. Der Prozess des Imports des öffentlichen Schlüssels in das UEFI-Schlüsselrepository wird durch Ausführen der folgenden Befehle gestartet:
+
...
. Die Software mokutil fordert während des Importvorgangs ein temporäres Passwort an, das für diesen Schlüssel verwendet werden soll.
. Es sollte überprüft werden, ob der Importvorgang mit dem  `mokutil --list-new` gestartet wurde, und anschließend das System neu gestartet werden. Der Bootloader startet den EFI MOK Manager.
. Über die Menüs auf dem Bildschirm lässt sich der SCST-Kernelmodulschlüssel aktivieren. Nach dem Systemstart  `systemctl status mediator-scst` ausführen. Sobald der Dienst gestartet ist, sind die SCST-Kernelmodule signiert.
