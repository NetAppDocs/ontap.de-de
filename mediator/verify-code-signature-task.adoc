...
[role="lead"]
NetApp empfiehlt, vor der Installation des Installationspakets für ONTAP Mediator die Signatur des ONTAP Mediator-Codes zu überprüfen. Dieser Vorgang ist jedoch optional.

.Bevor Sie beginnen
Bevor Sie die ONTAP Mediator-Code-Signatur überprüfen können, muss Ihr System die folgenden Anforderungen erfüllen.


NOTE: Wenn das System die folgenden Anforderungen nicht erfüllt, ist der Überprüfungsprozess nicht erforderlich und Sie können direkt zu gehenlink:install-mediator-pkg-task.html["Installieren Sie das Installationspaket für den ONTAP Mediator"].
...
`ontap-mediator-1.9.0`
 a| 
Die ausführbare Produktinstallation für Version 1.9.0
...
----
. Überprüfen Sie die `ontap-mediator-1.9.0.sig.tsr` `ontap-mediator-1.9.0.tsr` Dateien und mit den zugehörigen Zertifikaten:
+
`openssl ts -verify`
