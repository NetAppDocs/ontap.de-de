...
Wenn die VM, auf der ONTAP Mediator ausgeführt wird, mit einem neueren Kernel aktualisiert werden muss, kann dies zu Kompatibilitätsproblemen mit den von ONTAP Mediator verwendeten SCST-Kernelmodulen führen.  In diesem Szenario empfiehlt NetApp , SCST manuell zu deinstallieren und neu zu installieren.



=== Schritt 1: SCST manuell deinstallieren
...
=== Schritt 2: SCST manuell installieren
...
NOTE: Wenn Sie eine Erstinstallation durchführen und SCST vor der Installation von ONTAP Mediator vorinstallieren möchten, können Sie diesen Schritt überspringen.  Das ONTAP Mediator-Installationsprogramm wendet während der Installation alle relevanten Patches auf die SCST-Komponenten an.
