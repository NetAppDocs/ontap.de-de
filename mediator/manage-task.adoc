...
[NOTE]
====
* In ONTAP Mediator 1.8 und früher sind die CA-Dateien: `ca.key`, `ca.csr`, `ca.srl` Und `ca.crt`. Ab ONTAP Mediator 1.9 sind die erforderlichen Dateien `intermediate.key` , , `intermediate.csr` `intermediate.srl` und `intermediate.crt`.
