...
NOTE: Für SnapMirror Business Continuity (SM-BC)-Cluster müssen Sie die `intermediate.crt` Und `root_ca.crt` Zertifikate an einen ONTAP Cluster. Sehenlink:../snapmirror-active-sync/mediator-install-task.html["Konfigurieren Sie ONTAP Mediator und Cluster für SnapMirror Active Sync"] .
...
. Aktualisieren Sie die `/opt/netapp/lib/ontap_mediator/uvicorn/config.json` Datei.
+
...
----
+
.. Wenn Sie eine Zertifikatsstruktur verwenden, bei der die `root_ca.crt` Zertifikat bietet eine `intermediate.crt` Zertifikat, das die `ontap_mediator_server.crt` Zertifikat, erstellen Sie eine Zertifikatskette aus dem `intermediate.crt` Und `ontap_mediator_server.crt` Zertifikate:
+
