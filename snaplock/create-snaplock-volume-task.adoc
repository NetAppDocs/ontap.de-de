...
----
+
Eine vollständige Liste der Optionen finden Sie auf der man-Page für den Befehl. Für SnapLock-Volumes stehen folgende Optionen nicht zur Verfügung: `-nvfail`, `-atime-update`, , `-is-autobalance-eligible` `-space-mgmt-try-first` Und `vmalign`.
