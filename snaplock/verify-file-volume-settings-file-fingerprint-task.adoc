...
summary: Mit den Befehlen volume file fingerprint start und volume file fingerprint dump können Sie wichtige Informationen über Dateien und Volumes anzeigen, darunter den Dateityp, das Ablaufdatum des Volumes usw. 
---
= ONTAP SnapLock Einstellungen überprüfen
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Sie können die  `volume file fingerprint start` und  `volume file fingerprint dump` Befehle verwenden, um wichtige Informationen über Dateien und Volumes anzuzeigen, einschließlich des Dateityps (regulär, WORM oder WORM-anhängbar), des Ablaufdatums des Volumes und so weiter.

.Schritte
. Dateifingerabdruck generieren:
+
...
----
+
Der Befehl generiert eine Sitzungs-ID, die Sie als Eingabe für den  `volume file fingerprint dump` Befehl verwenden können.

+
[NOTE]
====
Sie können den  `volume file fingerprint show` Befehl mit der Sitzungs-ID verwenden, um den Fortschritt des Fingerabdruckvorgangs zu überwachen. Es sollte sichergestellt sein, dass der Vorgang abgeschlossen ist, bevor versucht wird, den Fingerabdruck anzuzeigen.

====
. Der Fingerabdruck der Datei wird angezeigt:
+
