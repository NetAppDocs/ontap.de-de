...
Eine gesperrte Snapshot Kopie auf dem Ziel-Volume, die zur Erfassung der divergierenden Daten erstellt wurde `snapmirror update -s snapshot`, kann mithilfe der CLI in die Quelle kopiert werden, indem der Befehl ausgeführt wird. Der nach dem Kopieren kopierte Snapshot wird weiterhin an der Quelle gesperrt.
