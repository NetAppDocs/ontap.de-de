...
SnapLock ist eine lizenzbasierte Funktion des NetApp ONTAP. Eine einzige Lizenz berechtigt Sie zur Verwendung von SnapLock im strengen Compliance-Modus, zur Erfüllung externer Vorgaben wie SEC Rule 17a-4 und einem gelockeren Enterprise-Modus, um die intern vorgeschriebenen Vorschriften zum Schutz digitaler Assets zu erfüllen. SnapLock Lizenzen sind Teil der link:../system-admin/manage-licenses-concept.html#licenses-included-with-ontap-one["ONTAP One"]Software Suite.
