...
SnapLock erfordert keine spezielle Hardware. SnapLock wird auf allen AFF und FAS Systemen sowie auf ONTAP Select unterstützt. SnapLock ist keine rein softwarebasierte Lösung, sondern eine integrierte Hardware- und Softwarelösung. Diese Auszeichnung ist wichtig für strenge WORM-Vorgaben wie SEC 17a-4, die eine integrierte Hardware- und Softwarelösung erfordert. Weitere Informationen finden Sie unter SEC-Interpretation: Elektronische Speicherung von Broker-Dealer Records.
...
Der Umfang des SnapLock Supports wurde mit ONTAP 9 Versionen weiterentwickelt. Die wichtigsten Erweiterungen sind:

Storage-Effizienz: Ab ONTAP 9.9 unterstützt SnapLock Storage-Effizienzfunktionen wie Data-Compaction, Volume-übergreifende Deduplizierung und anpassungsfähige Komprimierung für SnapLock Volumes und Aggregate. Weitere Informationen zur Storage-Effizienz finden Sie unter link:https://docs.netapp.com/us-en/ontap/volumes/index.html["Logisches Storage-Management – Übersicht mit der CLI"].

FlexGroup Volumes: Ab ONTAP 9.11.1 unterstützt SnapLock FlexGroup Volumes mit Ausnahme der gesetzlichen Aufbewahrungspflichten, ereignisbasierten Aufbewahrung und SnapLock für SnapVault. Ab ONTAP 9.12.1 unterstützt SnapLock für SnapVault FlexGroup Volumes.

SnapLock WORM Storage nutzt die NetApp Snapshot™ Technologie und kann SnapMirror ® Replikation sowie SnapVault® Backups als Basistechnologie für die Sicherung von Backup Recovery für Daten nutzen.
