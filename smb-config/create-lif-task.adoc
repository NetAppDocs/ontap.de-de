...
+
Bei der Verwendung von ONTAP 9.5 und früheren Versionen wird der `-data-protocol` Der Parameter muss angegeben werden, wenn die LIF erstellt wird, und kann später nicht geändert werden, ohne die Daten-LIF zu zerstören und neu zu erstellen.
