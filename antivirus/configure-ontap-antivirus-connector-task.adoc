...
. Stellen Sie sicher, dass das CA-Zertifikat (zusammen mit allen Zwischenzertifizierungsstellen bis hin zur Stammzertifizierungsstelle), das zum Signieren des Zertifikats für den NetApp -Server verwendet wurde, im Privacy Enhanced Mail (PEM)-Format vorliegt und den Namen „Ontap_CA.pem“ trägt. Platzieren Sie es im Installationsverzeichnis des Antivirus Connectors. Installieren Sie auf dem ONTAP System das CA-Zertifikat (zusammen mit allen Zwischenzertifizierungsstellen bis hin zur Stammzertifizierungsstelle), das zum Signieren des Clientzertifikats für den Antivirus Connector unter "ONTAP" verwendet wurde, als ein Zertifikat vom Typ "client-ca".
