...
* Der  `scan-queue-timeout`Wert sollte kleiner als der  `scanner-pool request-timeout`Wert sein. Ist er auf einen höheren Wert eingestellt, kann sich der Zugriff auf Dateien verzögern und schließlich zu einem Timeout führen. Um dies zu vermeiden, sollte der  `scan-queue-timeout`Wert auf 5 Sekunden weniger als der  `scanner-pool request-timeout`Wert konfiguriert werden. Informationen zum Ändern der  `scan-queue-timeout`Einstellungen sind in der Dokumentation des Scan-Engine-Anbieters zu finden. Der  `scanner-pool timeout`Wert kann im erweiterten Modus mit dem folgenden Befehl und dem entsprechenden Wert für den  `request-timeout`Parameter geändert werden: 
