...
.. Definieren Sie eine Regel für die SnapVault-Richtlinie, die festlegt, welche Snapshot Kopien während der Initialisierungs- und Aktualisierungsvorgänge übertragen werden: `snapmirror policy add-rule -vserver svm_name -policy policy_for_rule - snapmirror-label snapmirror-label -keep retention_count -schedule schedule`
