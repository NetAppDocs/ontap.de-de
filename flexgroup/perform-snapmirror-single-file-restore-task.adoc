...
summary: Ab ONTAP 9.8 ist die Wiederherstellung einer einzelnen Datei aus einem FlexGroup SnapMirror Vault oder von einem UDP-Ziel möglich. 
---
= Eine SnapMirror Single File Restore aus einem ONTAP FlexGroup Volume durchführen
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Ab ONTAP 9.8 ist die Wiederherstellung einer einzelnen Datei aus einem FlexGroup SnapMirror Vault oder von einem UDP-Ziel möglich.

.Über diese Aufgabe
* Eine Wiederherstellung von einem FlexGroup Volume beliebiger Geometrie auf ein FlexGroup Volume beliebiger Geometrie ist möglich.
* Pro Wiederherstellungsvorgang wird nur eine Datei unterstützt.
* Eine Wiederherstellung ist entweder auf das ursprüngliche Quell-FlexGroup-Volume oder auf ein neues FlexGroup-Volume möglich.
* Die Suche nach entfernten geschützten Dateien wird nicht unterstützt.
+
Die Wiederherstellung einer einzelnen Datei schlägt fehl, wenn die Quelldatei geschützt ist.

* Eine abgebrochene Wiederherstellung einer einzelnen Datei kann neu gestartet oder bereinigt werden.
* Ein fehlgeschlagener Wiederherstellungstransfer einer einzelnen Datei kann mit der  `clean-up-failure` Option des  `snapmirror restore` Befehls bereinigt werden.
+
...
* Die Erweiterung von FlexGroup Volumes wird unterstützt, wenn eine FlexGroup Single File Restore im Gange oder abgebrochen ist.


.Schritte
. Eine Datei aus einem FlexGroup Volume wiederherstellen: `snapmirror restore -destination-path destination_path -source-path source_path -file-list /f1 -throttle throttle -source-snapshot snapshot`
+
Im Folgenden wird ein Beispiel für eine FlexGroup Volume-Einzeldateiwiederherstellung dargestellt.
