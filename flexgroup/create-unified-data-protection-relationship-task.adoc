...
.. Definieren Sie eine Regel für die Mirror- und Vault-Richtlinie, die festlegt, welche Snapshot Kopien während Initialisierungs- und Aktualisierungsvorgängen übertragen werden: `snapmirror policy add-rule -vserver svm_name -policy policy_for_rule - snapmirror-label snapmirror-label -keep retention_count -schedule schedule`
