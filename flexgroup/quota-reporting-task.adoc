...
+
** In ONTAP 9 2 und ONTAP 9.1 kann der Quota-Zieltyp nur `user` oder `group` für FlexGroup-Volumes sein.
+
Der Tree Quotentyp wird für FlexGroup-Volumes in ONTAP 9.2 und ONTAP 9.1 nicht unterstützt.

** In ONTAP 9.3 und später kann der Quota-Zieltyp `user` , `group` oder `tree` für FlexGroup-Volumes sein.
