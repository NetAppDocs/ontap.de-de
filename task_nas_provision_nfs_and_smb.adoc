...
+
Alternativ können Sie auch weiterhin alle zusätzlichen erforderlichen Services wie Autorisierung, Servicequalität und Datensicherung aktivieren. Siehe  the volume configuration, Dann kehren Sie hier zurück, um die folgenden Schritte abzuschließen.
...
Nachdem Sie die Lautstärke gespeichert haben, kehren Sie zu zurück  Umfassendes Multiprotokoll-Provisioning für Windows und Linux Server.
