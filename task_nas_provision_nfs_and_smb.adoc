...
+
Nach der Überprüfung des Zugriffs können Sie link:task_nas_provision_export_policies.html["Beschränken Sie den Client-Zugriff mit der Exportrichtlinie des Volumes, beschränken Sie den Client-Zugriff mit der Share-ACL"], und legen Sie alle gewünschten Eigentumsrechte und Berechtigungen für das exportierte und freigegebene Volume.
