...
.Beispiele
Mit dem folgenden Befehl wird ein neues Volume mit dem Namen users1 auf der SVM erstellt `vs1.example.com` Und das Aggregat `aggr1`. Der neue Band wird bei zur Verfügung gestellt `/users`. Das Volume ist 750 GB groß und seine Volumengarantie ist vom Typ Volume (standardmäßig).
