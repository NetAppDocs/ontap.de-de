...
+
FabricPool unterstützt nicht das Anbinden einer Cloud Tier an ein Aggregat, das Volumes mit einer anderen Speicherplatzgarantie als enthält `none`. Beispiel: Ein Volume mit einer Platzgarantie von `volume` (`-space-guarantee` `volume`) Wird nicht unterstützt.
