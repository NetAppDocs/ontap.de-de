...
---
= ONTAP FabricPool Volume-Tiering-Richtlinien
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
| Tiering-Richtlinie | Standardmäßiges Abrufverhalten 
...
on-read
 a| 
Wenn kalte Blöcke in einem Volume mit einer Cloud-Abrufrichtlinie, die auf "`On-Read`" gesetzt ist, zufällig oder sequenziell gelesen werden, werden sie zu heißen Blöcken und in die lokale Ebene geschrieben.

Anwendungen, die sequenzielle Lesevorgänge nutzen, lösen Rückschreibvorgänge auf die lokale Ebene aus, indem die Abrufrichtlinie für das Volume aus der Cloud auf „On-Read“ gesetzt wird. Dies kann für Anwendungen von Vorteil sein, die lokale Ebenenleistung für zuvor kalte Daten benötigen, die nun von aktiven Workloads gelesen werden.
...
niemals
 a| 
Wenn kalte Blöcke in einem Volume mit einer Cloud-Abrufrichtlinie, die auf „Nie“ gesetzt ist, gelesen werden, bleiben sie kalt und verbleiben auf der Cloud-Ebene. Sie werden nicht zurück auf die lokale Ebene geschrieben.

Die Einstellung der Cloud-Abrufrichtlinie auf „`Never`“ ist ähnlich wie die All Tiering-Richtlinie, da Daten nicht auf die lokale Ebene zurückkehren dürfen, unterscheidet sich jedoch von der All Tiering-Richtlinie dadurch, dass weiterhin die Einstellung „tiering-minimum-cooling-days“ des Volumes verwendet wird, anstatt die Daten so schnell wie möglich zu verschieben.

Beispielsweise würde ein Volume, das die Standardeinstellung der Auto Tiering-Richtlinie verwendet, Daten erst nach 31 Tagen Inaktivität als kalt markieren. Nach 31 Tagen würden die inaktiven Daten in den Objektspeicher verschoben und kämen beim Lesen nicht zurück, da die Cloud Retrieval Policy des Volumes auf "Never" gesetzt wurde.
...
promoten
 a| 
* Für die Tiering-Richtlinie "`none,`" werden alle gestuften Daten zurück auf die lokale Tier geschrieben.
* Für die Tiering-Richtlinie „`snapshot-only`“ werden gestaffelte, nicht-Snapshot-Daten zurück in die lokale Ebene geschrieben.


|===
.Beispiel für die Änderung der Cloud Retrieval-Richtlinie
