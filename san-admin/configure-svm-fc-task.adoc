...
.Bevor Sie beginnen
Sie müssen über eine FC-Lizenz (link:https://docs.netapp.com/us-en/ontap/system-admin/manage-licenses-concept.html#licenses-included-with-ontap-one["Im Lieferumfang von ONTAP One enthalten"]) verfügen und diese muss aktiviert sein. Wenn die FC-Lizenz nicht aktiviert ist, scheinen die LIFs und SVMs online zu sein, der Betriebsstatus lautet jedoch `down` . Der FC-Service muss aktiviert sein, damit Ihre LIFs und SVMs funktionsfähig sind. Zum Hosten der Initiatoren müssen Sie das einzelne Initiator-Zoning für alle FC-LIFs in der SVM verwenden.
