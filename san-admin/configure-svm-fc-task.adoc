...
.Bevor Sie beginnen
Sie müssen eine FC-Lizenz (link:https://docs.netapp.com/us-en/ontap/system-admin/manage-licenses-concept.html#licenses-included-with-ontap-one["Im Lieferumfang von ONTAP One enthalten"]) Und es muss aktiviert sein. Wenn die FC-Lizenz nicht aktiviert ist, werden die LIFs und SVMs online angezeigt, während der Betriebsstatus jedoch lautet `down`. Der FC-Service muss aktiviert sein, damit Ihre LIFs und SVMs funktionsfähig sind. Zum Hosten der Initiatoren müssen Sie das einzelne Initiator-Zoning für alle FC-LIFs in der SVM verwenden.
