...
summary: ONTAP unterstützt bestimmte VMware vStorage APIs für Array-Integrationsfunktionen, wenn auf dem ESX-Host ESX 4.1 oder höher ausgeführt wird. Diese Funktionen tragen dazu bei, Operationen vom ESX-Host auf das Speichersystem auszulagern und den Netzwerkdurchsatz zu erhöhen. Der ESX-Host aktiviert die Funktionen in der entsprechenden Umgebung automatisch. 
---
