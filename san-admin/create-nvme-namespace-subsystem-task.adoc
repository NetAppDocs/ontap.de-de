...
[role="lead"]
Bei der Bereitstellung von Speicher ermöglichen Sie Ihren NVMe-Hosts das Lesen und Schreiben von Daten auf ONTAP-Speichersystemen. Für Systeme, die das NVMe-Protokoll verwenden, müssen Sie einen oder mehrere NVMe-Namespaces und Subsysteme erstellen. Jeder Namespace kann dann einem NVMe-Subsystem zugeordnet werden, um den Datenzugriff von Ihrem Host zu ermöglichen.
