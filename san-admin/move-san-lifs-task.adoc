...
summary: Wenn ein Knoten offline genommen werden muss, können Sie eine SAN-LIF verschieben, um deren Konfigurationsinformationen, wie z. B. die WWPN, zu erhalten und eine Neuzonierung der Switch-Fabric zu vermeiden. Da eine SAN-LIF vor dem Verschieben offline genommen werden muss, muss der Host-Datenverkehr auf Host-Multipathing-Software zurückgreifen, um unterbrechungsfreien Zugriff auf die LUN zu ermöglichen. Sie können SAN-LIFs auf jeden beliebigen Knoten in einem Cluster verschieben, aber Sie können die SAN-LIFs nicht zwischen Storage Virtual Machines verschieben. 
---
