...
NOTE: Ein SAN-LIF wird nicht migriert (es sei denn, Sie verschieben ihn nach dem Verbindungsausfall manuell). Stattdessen leitet die Multipathing-Technologie auf dem Host den Datenverkehr zu einem anderen LIF um. Weitere Informationen finden Sie unter link:../san-admin/index.html["SAN Administration"].
