...
summary: In einem Rechenzentrum können Sie Neighbor-Discovery-Protokolle verwenden, um die Netzwerkverbindungen zwischen zwei physischen oder virtuellen Systemen und deren Netzwerkschnittstellen anzuzeigen. ONTAP unterstützt zwei Neighbor-Discovery-Protokolle, Cisco Discovery Protocol und Link Layer Discovery Protocol. 
---
...
Mehrere Switches von Anbietern unterstützen CDP nicht. Siehe dielink:https://kb.netapp.com/onprem/ontap/da/NAS/ONTAP_device_discovery_shows_nodes_instead_of_the_switch["NetApp Knowledge Base: ONTAP Geräteerkennung zeigt Knoten anstelle des Switches an"^] für weitere Details.
