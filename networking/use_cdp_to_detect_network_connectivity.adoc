...
Switches verschiedener Hersteller unterstützen CDP nicht.  Weitere Informationen finden Sie im Knowledge Base-Artikel https://kb.netapp.com/onprem/ontap/da/NAS/ONTAP_device_discovery_shows_nodes_instead_of_the_switch["Bei der ONTAP-Geräteerkennung werden Nodes anstelle des Switches angezeigt"^] Entnehmen.

Es gibt zwei Möglichkeiten, dieses Problem zu lösen:

* Deaktivieren Sie CDP, und aktivieren Sie LLDP, falls unterstützt. Siehe link:use_lldp_to_detect_network_connectivity.html["Verwenden Sie LLDP, um die Netzwerkverbindung zu erkennen"] Entnehmen.
