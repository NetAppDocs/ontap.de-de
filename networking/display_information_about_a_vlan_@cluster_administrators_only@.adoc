...
.Über diese Aufgabe
Sie können für jedes VLAN ausführliche Informationen anzeigen, indem Sie den angeben `-instance` Parameter. Sie können bestimmte Informationen anzeigen, indem Sie Feldnamen mithilfe der angeben `-fields` Parameter.

.Schritt
Zeigen Sie Informationen zu VLANs mithilfe des an `network port vlan show` Befehl. Mit dem folgenden Befehl werden Informationen zu allen VLANs im Cluster angezeigt:
