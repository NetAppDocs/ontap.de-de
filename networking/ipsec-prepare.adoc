...
summary: Ab ONTAP 9.8 haben Sie die Möglichkeit, Ihren Netzwerkverkehr mithilfe von IP-Sicherheit zu schützen. IPsec ist eine von mehreren Optionen zur Verschlüsselung von Daten während der Übertragung, die mit ONTAP verfügbar sind. Sie sollten IPsec konfigurieren, bevor Sie es in einer Produktionsumgebung verwenden. 
---
