...
NOTE: Der Trap für die Authentifizierungsausfältung ist standardmäßig deaktiviert. Sie müssen den Befehl verwenden `system snmp authtrap` , um den Trap zu aktivieren. Weitere Informationen finden Sie auf den man-Pages: link:../concepts/manual-pages.html["Befehlsreferenz für ONTAP"]
