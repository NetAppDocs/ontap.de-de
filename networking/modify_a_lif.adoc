...
* ONTAP verwendet das Network Time Protocol (NTP), um die Zeit im gesamten Cluster zu synchronisieren. Nach dem Ändern der LIF-IP-Adressen müssen Sie möglicherweise die NTP-Konfiguration aktualisieren, um Synchronisierungsfehler zu vermeiden. Weitere Informationen finden Sie imlink:https://kb.netapp.com/on-prem/ontap/Ontap_OS/OS-Issues/CONTAP-500629["NetApp Knowledge Base: NTP-Synchronisierung schlägt nach LIF-IP-Änderung fehl"^] .
