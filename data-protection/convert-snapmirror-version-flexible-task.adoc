...
====
+
Im folgenden Beispiel wird eine Disaster Recovery-Beziehung zwischen dem Quell-Volume in SnapMirror erstellt `volA` Ein `svm1` Und dem Ziel-Volume `volA_dst` Ein `svm_backup` Die Standardeinstellung wird verwendet `MirrorAllSnapshots` Richtlinie:
