...
[role="lead"]
Ab ONTAP 9.9.1 können Sie SnapMirror cloud nutzen, wenn Sie Ihre Datenschutz-Workflows in die Cloud überführen möchten. Anstelle herkömmlicher Backup-to-Tape-Architekturen können Sie Objektspeicher als alternatives Repository für die langfristige Datenaufbewahrung und Archivierung verwenden. Sie können Anbieter wie Amazon S3, Azure Blob, Google Cloud Storage, StorageGRID oder ONTAP S3 als Ihren Cloud-Objektspeicher verwenden.
...
+
** Bei Verwendung von mehr als 6 Buckets pro Cluster sollten Sie die URL-Konfiguration im Virtual-Hosted-Style für Anbieter nutzen, die dies unterstützen, darunter AWS S3, Alibaba Cloud, StorageGrid, Google Cloud Storage, IBM Cloud object storage, ONTAP S3 und andere S3-kompatible Anbieter. Diese Konfiguration wird empfohlen, um skalierbare Objektspeicher-Adressierung zu ermöglichen und zugrunde liegende Einschränkungen zu vermeiden.
