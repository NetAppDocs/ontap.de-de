...
NOTE: Der `schedule` Der Parameter ist beim Erstellen von synchronen SnapMirror Beziehungen nicht anwendbar.
...
NOTE: Die vordefinierte Richtlinie `SnapCenterSync` Ist des Typs `Sync`. Diese Richtlinie repliziert alle Snapshot Kopien, die zusammen mit erstellt werden `snapmirror-label` Von „App_konsistent“.
