...
[role="lead"]
Sie können ein Volume auf einen früheren Zeitpunkt wiederherstellen, indem Sie es aus einer Snapshot Kopie wiederherstellen. Sie können den Befehl verwenden `volume snapshot restore` , um den Inhalt eines Volume aus einer Snapshot Kopie wiederherzustellen.
...
.Schritte
Sie können System Manager oder die ONTAP CLI verwenden.
