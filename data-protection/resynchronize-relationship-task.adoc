...
NOTE: Verwendung `-quick-resync` kann aufgrund der Reduzierung der Speichereffizienz bei übertragenen Datenblöcken zusätzlichen Speicherplatz auf dem Resynchronisierungszielvolume beanspruchen. Dieser zusätzliche Speicherplatzverbrauch wird im Rahmen der Inline- oder Post-Replikations-Speichereffizienzanwendung auf dem Resynchronisierungsziel kompensiert.

+
Der `-quick-resync` Der Parameter ist optional. Sie können die Schnellsynchronisierung aktivieren oder deaktivieren, indem Sie die folgende Option verwenden: `-quick-resync true|false` Parameter mit dem `snapmirror resync` Befehl.

+
Für weitere Informationen über `-quick-resync`siehe die link:https://docs.netapp.com/us-en/ontap-cli/snapmirror-resync.html["ONTAP-Befehlsreferenz"^]Die
...
* link:resync-svm-destination-task.html["Synchronisieren Sie die Daten auf einer ONTAP SnapMirror Ziel-SVM erneut"]
