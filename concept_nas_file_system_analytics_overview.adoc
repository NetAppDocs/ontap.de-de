...
| Visualisierung in System Manager | X | X | X | X | X | X 


| Kapazitätsanalysen | X | X | X | X | X | X 


| Informationen zu inaktiven Daten | X | X | X | X | X | X 


| Unterstützung für Volumes, die aus Data ONTAP 7-Mode migriert wurden | X | X | X | X | X |  


| Möglichkeit zum Anpassen inaktiver Perioden in System Manager | X | X | X | X | X |  


| Aktivitätenverfolgung auf Volume-Ebene | X | X | X | X |  |  


| Vorgangsverfolgungsdaten in CSV herunterladen | X | X | X | X |  |  


| Aktivitätsverfolgung auf SVM-Ebene | X | X | X |  |  |  


| Zeitachse | X | X | X |  |  |  


| Nutzungsanalysen | X | X |  |  |  |  


| Option zum Aktivieren der Dateisystemanalyse standardmäßig | X |  |  |  |  |  
|===
