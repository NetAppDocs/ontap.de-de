...
| xref:../snaplock/snapshot-lock-concept.html#enable-snapshot-copy-locking-when-creating-a-volume[Manipulationssichere Snapshot Sperrung] | System Manager kann zum Sperren eines Snapshots auf einem nicht-SnapLock-Volume verwendet werden, um so Schutz vor Ransomware-Angriffen zu bieten. 
