...
xref:../san-admin/enable-space-allocation.html[Der NVMe-Leerraum ist standardmäßig aktiviert]
 a| 
Die Freigabe von Speicherplatz (auch „Hole Punching“ und „Unmap“ genannt) ist für NVMe-Namespaces standardmäßig aktiviert. Durch die Freigabe von Speicherplatz kann ein Host die Zuweisung nicht verwendeter Blöcke aus Namespaces aufheben, um Speicherplatz freizugeben. Dadurch wird die allgemeine Speichereffizienz erheblich verbessert, insbesondere bei Dateisystemen mit hohem Datenumsatz.
