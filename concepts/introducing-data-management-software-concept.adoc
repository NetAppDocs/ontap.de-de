...
ONTAP Implementierungen werden auf von NetApp entwickelten FAS-, AFF- oder C-Series Appliances auf Standard-Hardware (ONTAP Select) und in Private, Public oder Hybrid Clouds (NetApp Private Storage oder Cloud Volumes ONTAP) ausgeführt. Spezialisierte Implementierungen bieten erstklassige konvergente Infrastrukturen (FlexPod-Datacenter) und Zugriff auf Storage-Arrays von Drittanbietern (FlexArray-Virtualisierung).
