...
[role="lead"]
_SnapMirror cloud_ ist eine Backup und Recovery-Technologie für ONTAP Nutzer, die ihre Datensicherungsprozesse in die Cloud oder auf eine lokale Objektspeicherlösung verlagern möchten. Organisationen, die von herkömmlichen Backup-to-Tape-Architekturen abkehren, können Objektspeicher als alternatives Repository für die langfristige Datenaufbewahrung und Archivierung verwenden. SnapMirror cloud bietet ONTAP zu Objektspeicher Replikation als Teil einer inkrementellen Forever-Backup-Strategie.
