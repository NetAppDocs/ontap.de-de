...
NOTE: Für Cluster-Administratorkonten wird die Zertifikatauthentifizierung nur mit unterstützt `http` Und `ontapi` Applikationen unterstützt. Bei SVM-Administratorkonten wird die Zertifikatauthentifizierung nur von unterstützt `ontapi` Applikation.
