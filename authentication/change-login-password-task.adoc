...
NOTE: Mit dem[`security login role config modify` Befehl können Sie die Passwortregeln für Konten ändern, die einer bestimmten Rolle zugeordnet sind. Weitere Informationen zum Befehl Link:https://docs.NetApp.com/US-en/ONTAP-cli/security-login-role-config-modify.html^] finden[`security login role config modify` Sie in der ONTAP-Befehlsreferenz.
