...
|===
Sie stellen diese Werte dem zur Verfügung `security certificate install` Befehl bei der Installation eines digitalen CA-signierten Zertifikats zur Authentifizierung des Clusters oder der SVM als SSL-Server. In der folgenden Tabelle sind nur die für diesen Leitfaden relevanten Optionen aufgeführt.
...
====
In der folgenden Tabelle sind nur die für diesen Leitfaden relevanten Optionen aufgeführt:
