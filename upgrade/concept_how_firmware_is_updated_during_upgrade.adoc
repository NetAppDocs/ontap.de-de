...
Auf Wunsch können Sie auch link:../update/firmware-task.html["Aktualisieren Sie die Firmware manuell"] Zwischen ONTAP-Upgrades. Wenn Sie Active IQ haben, können Sie dies link:https://activeiq.netapp.com/system-firmware/["Zeigen Sie die Liste der derzeit in Ihrem ONTAP-Image enthaltenen Firmware-Versionen an"^].
