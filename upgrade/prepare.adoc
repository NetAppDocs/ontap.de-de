...
. Überprüfen Sie die unterstützten ONTAP link:concept_upgrade_paths.html#supported-upgrade-paths-for-on-premises-ontap-and-ontap-select["Upgrade-Pfade"] Um festzustellen, ob Sie ein direktes Upgrade durchführen können oder ob Sie das Upgrade schrittweise durchführen müssen.
