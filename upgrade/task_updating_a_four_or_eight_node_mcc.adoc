...
---
= Manuelle ONTAP NDU auf einer Vier- oder Acht-Knoten MetroCluster Konfiguration
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Ein manuelles, unterbrechungsfreies Upgrade (NDU) einer vier- oder acht-Knoten MetroCluster Konfiguration umfasst die Vorbereitung des Updates, die gleichzeitige Aktualisierung der DR-Paare in jeder der ein oder zwei DR-Gruppen sowie die Durchführung von Aufgaben nach dem Upgrade.

.Über diese Aufgabe
* Für diese Aufgabe muss die ONTAP command line interface (CLI) verwendet werden.
