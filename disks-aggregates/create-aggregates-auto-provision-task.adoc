...
summary: 'Sie können eine Empfehlung von ONTAP zum automatischen Hinzufügen einer lokalen Ebene akzeptieren (und ein Aggregat mit automatischer Bereitstellung erstellen) und ONTAP die lokale Ebene hinzufügen lassen.' 
---
= Fügen Sie automatisch lokale Tiers hinzu (erstellen Sie Aggregate mit automatischer Bereitstellung) in ONTAP
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
