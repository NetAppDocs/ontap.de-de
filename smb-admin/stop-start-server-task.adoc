...
summary: Sie können den CIFS-Server auf einer SVM stoppen, was nützlich ist, wenn Sie Aufgaben ausführen, während Benutzer nicht über SMB-Freigaben auf Daten zugreifen. Sie können den SMB-Zugriff wiederherstellen, indem Sie den CIFS-Server starten. Durch das Stoppen des CIFS-Servers können Sie außerdem die auf der Storage Virtual Machine zulässigen Protokolle ändern. 
---
