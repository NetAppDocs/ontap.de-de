...
summary: Es sind mehrere Schritte erforderlich, um Überwachungsrichtlinien auf NTFS-Dateien und -Ordner mit der ONTAP-CLI anzuwenden. Zuerst erstellen Sie einen NTFS-Sicherheitsdeskriptor und fügen diesem SACLs hinzu. Als Nächstes erstellen Sie eine Sicherheitsrichtlinie und fügen Richtlinienaufgaben hinzu. Anschließend wenden Sie die Sicherheitsrichtlinie auf eine Storage Virtual Machine an. 
---
