...
summary: Die Option, die es Ihnen ermöglicht, Dynamic Access Control zum Schutz von Objekten auf Ihrem CIFS-Server zu verwenden, ist standardmäßig deaktiviert. Sie müssen die Option aktivieren, wenn Sie Dynamic Access Control auf Ihrem CIFS-Server verwenden möchten. Wenn Sie sich später entscheiden, Dynamic Access Control nicht mehr zum Schutz von auf dem CIFS-Server gespeicherten Objekten zu verwenden, können Sie die Option deaktivieren. 
---
