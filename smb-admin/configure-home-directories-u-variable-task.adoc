...
summary: Sie können eine Home Directory-Konfiguration erstellen, in der Sie den Freigabenamen mithilfe der Windows-Benutzernamensvariablen festlegen, während Sie die UNIX-Benutzernamensvariable verwenden, um den relativen Pfad zum Home Directory-Pfad anzugeben. Benutzer können sich dann über dynamisch erstellte Freigaben, die mit ihrem Windows-Benutzernamen erstellt wurden, mit ihrer Home Directory-Freigabe verbinden, ohne den tatsächlichen Namen oder Pfad des Home Directory zu kennen. 
---
