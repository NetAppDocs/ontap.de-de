...
summary: Sie können die Dateisicherheitseinstellungen überprüfen, um sicherzustellen, dass die Dateien oder Ordner auf der Storage Virtual Machine, auf die Sie die Sicherheitsrichtlinie angewendet haben, die gewünschten Einstellungen aufweisen. 
---
= Die Sicherheit von ONTAP SMB-Dateien überprüfen
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
.Über diese Aufgabe
Sie müssen den Namen der SVM, die die Daten enthält, sowie den Pfad zu den Dateien und Ordnern angeben, deren Sicherheitseinstellungen überprüft werden sollen. Der optionale  `-expand-mask`Parameter kann verwendet werden, um detaillierte Informationen zu den Sicherheitseinstellungen anzuzeigen.
