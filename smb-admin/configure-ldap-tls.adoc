---
permalink: smb-admin/configure-ldap-tls.html 
sidebar: sidebar 
keywords: ldap, ssl, tls, secure, securing, active, directory, communication, export, copy, certificate, service, self-signed, root, ca, file, ascii, text, storage, virtual, machine, svm, export a copy of the self-signed root ca certificate 
summary: Konfigurieren Sie LDAP über TLS für ONTAP SMB-Server, um die Kommunikation zwischen dem SMB-Server und Active Directory LDAP-Servern zu sichern. 
---
= LDAP über TLS für ONTAP SMB-Server konfigurieren
:allow-uri-read: 
:icons: font
:imagesdir: ../media/


[role="lead"]
Konfigurieren Sie LDAP über TLS für ONTAP SMB-Server, um die Kommunikation zwischen dem SMB-Server und Active Directory LDAP-Servern zu sichern.



[[step-1-export-self-signed-root-ca-certificates-for-ontap-smb-svms]]
== Schritt 1: Selbstsignierte Stammzertifizierungsstellenzertifikate für ONTAP SMB SVMs exportieren

Um LDAP über SSL/TLS zur Sicherung der Active Directory-Kommunikation zu verwenden, muss zunächst eine Kopie des selbstsignierten Stammzertifikats des Active Directory Certificate Service in eine Zertifikatsdatei exportiert und in eine ASCII-Textdatei konvertiert werden. Diese Textdatei wird von ONTAP verwendet, um das Zertifikat auf der Storage Virtual Machine (SVM) zu installieren.

Der Active Directory-Zertifikatdienst muss für die Domäne, zu der der CIFS-Server gehört, bereits installiert und konfiguriert sein. Informationen zur Installation und Konfiguration von Active Directory Certificate Services finden Sie in der Dokumentation http://technet.microsoft.com/en-us/library/["Microsoft TechNet Bibliothek: technet.microsoft.com"]

[NOTE]
====
Ab ONTAP 9.13.1 ist, wenn  `-use-start-tls-for-ad-ldap` oder  `-use-ldaps-for-ad-ldap` für den SMB-Server aktiviert ist, sicherzustellen, dass die erforderlichen CA-Zertifikate für vertrauenswürdige Domänen sowie für die Heimatdomäne installiert sind. ONTAP muss TLS- oder LDAPS-Verbindungen zu Domänencontrollern vertrauenswürdiger Domänen herstellen können, um deren Standortinformationen zu ermitteln.

====
.Schritt
. Ein Root-CA-Zertifikat des Domänencontrollers im  `.pem` Textformat erhalten.
+
http://technet.microsoft.com/en-us/library/["Microsoft TechNet Bibliothek: technet.microsoft.com"]



.Was kommt als Nächstes
Das Zertifikat auf der SVM installieren.



[[step-2-install-self-signed-root-ca-certificates-on-the-ontap-smb-svm]]
== Schritt 2: Installieren Sie selbstsignierte Stammzertifikate der Zertifizierungsstelle auf der ONTAP SMB SVM

Wenn bei der Anbindung an LDAP-Server eine LDAP-Authentifizierung mit TLS erforderlich ist, muss zunächst das selbstsignierte Root-CA-Zertifikat auf der SVM installiert werden.

.Über diese Aufgabe
Alle Anwendungen innerhalb von ONTAP, die TLS-Kommunikation nutzen, können den Status digitaler Zertifikate mithilfe des Online Certificate Status Protocol (OCSP) überprüfen. Wenn OCSP für LDAP über TLS aktiviert ist, werden widerrufene Zertifikate zurückgewiesen und die Verbindung schlägt fehl.

.Schritte
. Das selbstsignierte Stammzertifikat der Zertifizierungsstelle installieren:
+
.. Beginnen Sie die Zertifikatsinstallation:
+
[source, cli]
----
security certificate install -vserver <SVM_name> -type server-ca
----
+
Die Konsolenausgabe zeigt folgende Meldung an: `Please enter Certificate: Press <Enter> when done`

.. Öffnen Sie die  `.pem` Zertifikatsdatei mit einem Texteditor, das Zertifikat einschließlich der Zeilen kopieren, die mit  `-----BEGIN CERTIFICATE-----` beginnen und mit  `-----END CERTIFICATE-----` enden, und das Zertifikat anschließend nach der Eingabeaufforderung einfügen.
.. Es wird überprüft, ob das Zertifikat korrekt angezeigt wird.
.. Die Installation wird durch Drücken der Eingabetaste abgeschlossen.


. Überprüfen Sie, ob das Zertifikat installiert ist:
+
[source, cli]
----
security certificate show -vserver <SVM_name>
----


.Verwandte Informationen
* link:https://docs.netapp.com/us-en/ontap-cli/security-certificate-install.html["Sicherheitszertifikat installieren"^]
* link:https://docs.netapp.com/us-en/ontap-cli/security-certificate-show.html["Sicherheitszertifikat anzeigen"^]




[[step-3-enable-ldap-over-tls-on-the-ontap-smb-server]]
== Schritt 3: LDAP über TLS auf dem ONTAP SMB-Server aktivieren

Bevor Ihr SMB-Server TLS für die sichere Kommunikation mit einem Active Directory LDAP-Server verwenden kann, müssen die Sicherheitseinstellungen des SMB-Servers geändert werden, damit LDAP über TLS genutzt werden kann.

Ab ONTAP 9.10.1 wird die LDAP-Kanalbindung standardmäßig sowohl für Active Directory (AD) als auch für Namensdienst-LDAP-Verbindungen unterstützt. ONTAP versucht die Kanalbindung mit LDAP-Verbindungen nur, wenn Start-TLS oder LDAPS aktiviert ist und die Sitzungssicherheit entweder auf Signieren oder Versiegeln eingestellt ist. Um die LDAP-Kanalbindung mit AD-Servern zu deaktivieren oder wieder zu aktivieren, kann der  `-try-channel-binding-for-ad-ldap` Parameter mit dem  `vserver cifs security modify` Befehl verwendet werden.

Weitere Informationen sind unter folgendem Link verfügbar:

* link:../nfs-admin/using-ldap-concept.html["Informationen zu LDAP für ONTAP NFS SVMs"]
* link:https://support.microsoft.com/en-us/topic/2020-ldap-channel-binding-and-ldap-signing-requirements-for-windows-ef185fb8-00f7-167d-744c-f299a66fc00a["LDAP-Kanalbindungs- und LDAP-Signaturanforderungen für Windows 2020"^].


.Schritte
. Konfigurieren Sie die SMB-Server-Sicherheitseinstellung, die eine sichere LDAP-Kommunikation mit Active Directory LDAP-Servern ermöglicht:
+
[source, cli]
----
vserver cifs security modify -vserver <SVM_name> -use-start-tls-for-ad-ldap true
----
+
[NOTE]
====
Ab ONTAP 9.13.1 verwendet ONTAP, wenn die sichere LDAP-Kommunikation für Active Directory-Verbindungen aktiviert ist, diese sicheren Verbindungen auch für die Kontaktaufnahme mit Domänencontrollern vertrauenswürdiger Domänen. Wenn die erforderlichen Zertifikate für Domänencontroller vertrauenswürdiger Domänen nicht installiert sind, kann ONTAP möglicherweise keine Standortinformationen zu vertrauenswürdigen Domänen ermitteln.

====
. Überprüfen Sie, ob die LDAP-über-TLS-Sicherheitseinstellung auf  `true` gesetzt ist:
+
[source, cli]
----
vserver cifs security show -vserver <SVM_name>
----
+
[NOTE]
====
Wenn die SVM denselben LDAP-Server für die Abfrage von Namenszuordnungen oder anderen UNIX-Informationen (wie Benutzern, Gruppen und Netzgruppen) verwendet, muss auch die Option  `-use-start-tls` mit dem Befehl  `vserver services name-service ldap client modify` geändert werden.

====

