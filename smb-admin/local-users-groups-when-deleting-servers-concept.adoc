...
summary: Der Standardsatz lokaler Benutzer und Gruppen wird bei der Erstellung eines CIFS-Servers erstellt und ist der Storage Virtual Machine zugeordnet, die den CIFS-Server hostet. SVM-Administratoren können jederzeit lokale Benutzer und Gruppen erstellen. Sie müssen wissen, was mit den lokalen Benutzern und Gruppen geschieht, wenn Sie den CIFS-Server löschen. 
---
