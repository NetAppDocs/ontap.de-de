...
summary: 'Standardmäßig ruft ONTAP alle 90 Minuten Änderungen an Gruppenrichtlinienobjekten (GPOs) ab und wendet sie an. Sicherheitseinstellungen werden alle 16 Stunden aktualisiert. Wenn Sie GPOs aktualisieren möchten, um neue GPO-Richtlinieneinstellungen anzuwenden, bevor ONTAP sie automatisch aktualisiert, können Sie eine manuelle Aktualisierung auf einem CIFS-Server mit einem ONTAP-Befehl auslösen.' 
---
