...
summary: Ein Symbol-Link ist eine in einer UNIX-Umgebung erstellte Datei, die auf eine andere Datei oder ein anderes Verzeichnis verweist. Wenn ein Client auf einen Symbol-Link zugreift, wird der Client zur Zieldatei oder zum Zielverzeichnis weitergeleitet, auf das sich der Symbol-Link bezieht. ONTAP unterstützt relative und absolute Symbol-Links, einschließlich Widelinks. 
---
