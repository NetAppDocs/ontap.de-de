...
summary: Oplocks ermöglichen es Clients, Dateien zu sperren und Inhalte lokal zwischenzuspeichern, was die Leistung bei Dateivorgängen verbessern kann. Oplocks sind für SMB-Freigaben auf Storage Virtual Machines aktiviert. In manchen Fällen möchten Sie Oplocks möglicherweise deaktivieren. Sie können Oplocks für jede Freigabe einzeln aktivieren oder deaktivieren. 
---
