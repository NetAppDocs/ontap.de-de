...
summary: Auto Location nutzt automatische Knotenverweise von SMB, um die Leistung von SMB-Clients auf Storage Virtual Machines zu steigern. Automatische Knotenverweise leiten den anfragenden Client automatisch zu einem LIF auf der Knoten-SVM weiter, die das Volume hostet, in dem sich die Daten befinden, was zu verbesserten Client-Antwortzeiten führen kann. 
---
