...
. Stellen Sie sicher, dass der Benutzer- und Gruppenzugriff sowohl auf den Quell- als auch auf den Zielspeicher-VMs korrekt ist: Wählen Sie *Speicher > Speicher-VMs*, wählen Sie dann die Speicher-VM und dann *Einstellungen* aus. Wählen Sie schließlich image:icon_pencil.gif["Symbol bearbeiten"] unter *S3* aus.
+
