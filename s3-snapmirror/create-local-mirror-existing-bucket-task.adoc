...
summary: Sie können jederzeit damit beginnen, bestehende S3-Buckets auf demselben Cluster zu schützen, beispielsweise wenn Sie eine S3-Konfiguration von einer Version vor ONTAP 9.10.1 aktualisiert haben. Sie können gespiegelte Daten in einen Bucket in einer anderen Storage-VM oder in derselben Storage-VM wie die Quelle spiegeln. 
---
