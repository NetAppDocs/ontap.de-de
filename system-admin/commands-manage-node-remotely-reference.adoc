...
Erfahren Sie mehr über die `help` Befehlsoptionen für SP und BMC in link:online-help-at-sp-bmc-cli-task.html["Zeigen Sie die Hilfeinformationen in der ONTAP SP/BMC-CLI an"].

Erfahren Sie mehr über die entsprechende ONTAP BMC CLI in link:bmc-cli-commands-reference.html["Unterstützte CLI-Befehle für den ONTAP Baseboard-Verwaltungscontroller"].
