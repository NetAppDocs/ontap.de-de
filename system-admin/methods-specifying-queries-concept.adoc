...
Sie können mehrere Abfrageoperatoren in einer Befehlszeile verwenden. Beispiel: Mit dem Befehl `volume show -size >1GB -percent-used <50 -vserver !vs1` werden alle Volumes angezeigt, die größer als 1 GB sind, zu weniger als 50 % genutzt werden und nicht in der Storage Virtual Machine (SVM) mit dem Namen „`vs1`“.
