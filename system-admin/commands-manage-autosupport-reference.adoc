...
[NOTE]
====
Mit dem `-force` Parameter können Sie eine Meldung senden, auch wenn AutoSupport deaktiviert ist. Verwenden Sie den `-uri` Parameter, um die Meldung an das angegebene Ziel statt an das konfigurierte Ziel zu senden.
