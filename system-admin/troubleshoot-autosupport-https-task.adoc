...
+
Der Zugriff sollte erfolgreich sein. Wenn nicht, stellen Sie sicher, dass alle Firewalls richtig konfiguriert sind, um HTTPS- und DNS-Verkehr zuzulassen, und dass der Proxyserver richtig konfiguriert ist. Weitere Informationen zum Konfigurieren der statischen Namensauflösung für support.netapp.com finden Sie imlink:https://kb.netapp.com/Advice_and_Troubleshooting/Data_Storage_Software/ONTAP_OS/How_would_a_HOST_entry_be_added_in_ONTAP_for_support.netapp.com%3F["NetApp Knowledge Base: Wie wird in ONTAP ein HOST-Eintrag für support.netapp.com hinzugefügt?"^]
