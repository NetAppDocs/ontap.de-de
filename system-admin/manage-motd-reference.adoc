...
summary: 'Sie können die Nachricht des Tages (MOTD) auf Cluster- oder SVM-Ebene verwalten. Standardmäßig ist die für den Cluster konfigurierte MOTD auch für alle SVMs aktiviert. Zusätzlich kann für jede SVM eine MOTD auf SVM-Ebene konfiguriert werden. Die MOTD auf Cluster-Ebene kann vom Cluster-Administrator für jede SVM aktiviert oder deaktiviert werden.' 
---
