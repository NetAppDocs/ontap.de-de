...
summary: Um einem SSL-Server die Authentifizierung des Clusters oder der Storage Virtual Machine als SSL-Client zu ermöglichen, installieren Sie ein digitales Zertifikat mit dem Client-Typ auf dem Cluster oder der SVM. Anschließend stellen Sie das Client-CA-Zertifikat dem SSL-Serveradministrator zur Installation auf dem Server zur Verfügung. 
---
