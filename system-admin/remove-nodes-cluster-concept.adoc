...
NOTE: Wenn Sie diesen Schritt am ersten Knoten eines HA-Paares durchführen, sollten Sie nur diesen Knoten als nicht geeignet markieren. Ändern Sie den Status des HA-Partners nicht. Wenn der Partnerknoten als neuer Master ausgewählt wird, müssen Sie überprüfen, ob er Epsilon enthält, bevor Sie ihn für ungültig erklären. Dies geschieht, wenn Sie diese Schritte wiederholen, um den Partnerknoten zu entfernen.
