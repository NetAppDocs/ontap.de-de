...
Die in der Clustershell unterstützten Nodeshell-Optionen können über die folgende Methode aufgerufen werden: `vserver options clustershell` Befehl. Um diese Optionen anzuzeigen, können Sie die Clustershell-CLI mit folgender Abfrage verwenden: `vserver options -vserver _nodename_or_clustername_ -option-name ?`
