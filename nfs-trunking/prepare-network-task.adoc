...
** Sie können eine IP-Adresse mit dem angeben `-address` Und `-netmask` Optionen, nicht mit `-subnet` Option.
** Wenn Sie IP-Adressen zuweisen, müssen Sie möglicherweise eine Standardroute zu einem Gateway konfigurieren, wenn Clients oder Domänencontroller in einem anderen IP-Subnetz vorhanden sind. Der `network route create` Die man-Page enthält Informationen zum Erstellen einer statischen Route in einer SVM.
...
+
Mit dem folgenden Befehl wird erstellt `lif-A` Für die SVM `vs1`, Am Hafen `e0c` Des Node `cluster1_01`:
