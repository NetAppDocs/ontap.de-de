...
summary: Die ausgelagerte Datenübertragung, auch bekannt als Copy-Offload, ermöglicht direkte Datenübertragungen innerhalb oder zwischen kompatiblen Speichermedien, ohne die Daten über den Host-Computer zu übertragen. ONTAP ODX Copy-Offload bietet Ihnen Leistungsvorteile beim Ausführen von Kopiervorgängen auf Ihrem Anwendungsserver über SMB-Installation. 
---
