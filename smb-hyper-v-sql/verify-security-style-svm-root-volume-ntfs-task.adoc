...
summary: Um unterbrechungsfreie Vorgänge von Hyper-V und SQL Server über SMB zu gewährleisten, müssen Volumes mit dem NTFS-Sicherheitsstil erstellt werden. Da der Sicherheitsstil des Stammvolumes standardmäßig auf Volumes angewendet wird, die auf der Storage Virtual Machine erstellt werden, sollte der Sicherheitsstil des Stammvolumes auf NTFS eingestellt werden. 
---
