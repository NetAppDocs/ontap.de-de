...
----
+
Weitere Informationen zum Befehl Link:https://docs.NetApp.com/US-en/ONTAP-cli/security-login-rest-role-create.html^] finden[`security login rest-role create` Sie in der ONTAP-Befehlsreferenz.

. Erstellen Sie eine LDAP-Benutzergruppe mit dem Befehl für die Sicherheitsanmeldung, und fügen Sie die neue benutzerdefinierte Rolle für den Zugriff auf den REST-API-Endpunkt des S3-Benutzers hinzu. Weitere Informationen zum Befehl Link:https://docs.NetApp.com/US-en/ONTAP-cli//security-login-create.html^] finden[`security login create` Sie in der ONTAP-Befehlsreferenz.
+
[listing]
----
