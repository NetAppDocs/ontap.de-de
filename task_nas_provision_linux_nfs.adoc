...
*** Klicken Sie auf *Weitere Optionen*, um die Konfiguration des Volumes anzupassen und so Services wie Autorisierung, Servicequalität und Datenschutz zu ermöglichen. Siehe  the volume configuration, Dann kehren Sie hier zurück, um die folgenden Schritte abzuschließen.
...
NOTE: Nachdem Sie die Lautstärke gespeichert haben, kehren Sie zu zurück  Um die Bereitstellung für Linux Server mithilfe von NFS abzuschließen.
