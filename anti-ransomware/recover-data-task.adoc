...
+
Erfahren Sie mehr über link:../volumes/create-flexclone-task.html["Erstellen eines FlexClone Volume aus einem Snapshot"] und link:../volumes/split-flexclone-from-parent-task.html["Abspaltung eines FlexClone von seinem Elternteil"].
...
NOTE: Wenn Sie die Aktivität bereits anhand der Schritte in link:respond-abnormal-task.html["Reagieren Sie auf ungewöhnliche Aktivität, die durch ONTAP ARP erkannt wurde"] als potenziellen Ransomware-Angriff eingestuft haben, müssen Sie hier nur noch die verdächtigen Dateitypen löschen.
