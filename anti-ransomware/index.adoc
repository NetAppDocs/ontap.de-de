...
[role="lead"]
Ab ONTAP 9.10.1 können ONTAP-Administratoren den autonomen Ransomware-Schutz (ARP) aktivieren, um eine Workload-Analyse in NAS-Umgebungen (NFS und SMB) durchzuführen, um proaktiv ungewöhnliche Aktivitäten zu erkennen und davor zu warnen, die auf einen Ransomware-Angriff hindeuten könnten. Ab ONTAP 9.17.1 unterstützt ARP auch Blockgeräte-Volumes, einschließlich SAN-Volumes mit LUNs oder NVMe-Namespaces oder NAS-Volumes mit virtuellen Festplatten von Hypervisoren wie VMware. Ab ONTAP 9.17.1P5 werden auch Hyper-V, KVM und OpenStack Hypervisoren unterstützt.
