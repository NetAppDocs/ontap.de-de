...
[role="lead"]
Ab ONTAP 9.10.1 können ONTAP-Administratoren den Autonomous Ransomware Protection (ARP) aktivieren, um eine Workload-Analyse in NAS-Umgebungen (NFS und SMB) durchzuführen, um proaktiv ungewöhnliche Aktivitäten zu erkennen und davor zu warnen, die auf einen Ransomware-Angriff hindeuten könnten. Ab ONTAP 9.17.1 unterstützt ARP auch Blockgeräte-Volumes, einschließlich SAN-Volumes mit LUNs oder NVMe-Namespaces sowie NAS-Volumes mit virtuellen Festplatten von Hypervisoren wie VMware. Ab ONTAP 9.17.1P5 werden auch Hyper-V, KVM und OpenStack Hypervisoren unterstützt, und ARP bietet einen Evaluierungsmodus für die in NAS-Volumes enthaltenen virtuellen Festplatten (siehe <<NAS-Umgebungen und Modustypen>>). ARP ist direkt in ONTAP integriert und gewährleistet so die integrierte Steuerung und Koordination mit den anderen ONTAP-Funktionen. ARP arbeitet in Echtzeit, verarbeitet Daten beim Schreiben oder Lesen im Dateisystem und erkennt und reagiert schnell auf potenzielle Ransomware-Angriffe.
