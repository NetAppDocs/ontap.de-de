...
[role="lead"]
Autonomous Ransomware Protection ist ab ONTAP 9.10.1 für NAS-Workloads verfügbar. Vor der Bereitstellung von ARP sollten Sie die empfohlenen Verwendungszwecke und unterstützten Konfigurationen sowie die Auswirkungen auf die Performance kennen.
...
| Volumes sind mit asynchronem SnapMirror geschützt | X | X |  |  


| SVMs gesichert mit asynchronem SnapMirror | X | X |  |  


| SVMs aktiviert für die Datenmigration | X | X |  |  


| FlexGroup Volumes | X |  |  |  


| Überprüfung durch mehrere Administratoren | X |  |  |  
|===
...
* SnapMirror zum asynchronen Betrieb (in ONTAP 9.10.1 bis 9.12.1) Asynchrones SnapMirror wird ab ONTAP 9.12.1 unterstützt. Weitere Informationen finden Sie unter <<snapmirror>>.)
