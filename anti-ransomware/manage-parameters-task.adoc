...
NOTE: Um die Anzahl falsch positiver Warnmeldungen zu reduzieren, gehen Sie zu *Speicher > Volumes > Sicherheit > Workload-Merkmale konfigurieren* und deaktivieren Sie *Neue Dateitypen überwachen*. Diese Einstellung ist in ONTAP 9.14.1 P7, 9.15.1 P1, 9.16.1 und später standardmäßig deaktiviert. Ab ONTAP 9.18.1 P2 zeigt der System Manager keine ARP-Workload-Merkmale mehr an, einschließlich Basis- und Anstiegsstatistiken. Anstiegsstatistiken sind weiterhin über ONTAP CLI und REST API Optionen für erweiterte Analysen verfügbar.
