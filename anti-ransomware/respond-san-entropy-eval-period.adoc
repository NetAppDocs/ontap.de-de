...
summary: Ab ONTAP 9.17.1 benötigt ARP eine Evaluierungsphase, um festzustellen, ob die SAN-Volume-Entropiewerte für den Schutz vor Ransomware geeignet sind. 
---
...
Sie können bestätigen, dass die Evaluierung aktiv ist, indem Sie den folgenden Befehl ausführen und einen Status von  `evaluation_period` bestätigen. Wenn ein Volume nicht für die Evaluierung geeignet ist, wird der Evaluierungsstatus nicht angezeigt.
...
* Die Arbeitslast ist für ARP geeignet. ARP setzt den adaptiven Schwellenwert automatisch auf mehr als 10 % des maximalen Verschlüsselungsanteils, der während des Auswertungszeitraums festgestellt wurde. ARP sammelt weiterhin Statistiken und erstellt regelmäßig ARP Snapshots.
* *Die Arbeitslast ist für ARP ungeeignet.* ARP setzt den adaptiven Schwellenwert automatisch auf den maximalen Verschlüsselungsprozentsatz, der während des Auswertungszeitraums festgestellt wurde. ARP setzt außerdem die Statistiken-Erfassung fort und erstellt regelmäßig ARP-Snapshots, jedoch empfiehlt das System letztendlich, ARP auf dem Volume zu deaktivieren.


.Ermittlung der Auswertungsergebnisse
Nach Ablauf des Evaluierungszeitraums stellt ARP den adaptiven Schwellenwert automatisch auf Basis der Evaluierungsergebnisse ein.

Die Auswertungsergebnisse lassen sich mit dem folgenden Befehl ermitteln. Die Eignung des Volumens wird im  `Block device detection status` Feld angezeigt:
