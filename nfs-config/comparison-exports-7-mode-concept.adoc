...
Wie Exporte definiert werden
 a| 
Exporte werden im definiert `/etc/exports` Datei:
 a| 
...
* Sie müssen einen separaten Eintrag in erstellen `/etc/exports` Für jeden Dateipfad oder qtree.
* Exporte bleiben nur bestehen, wenn sie im definiert sind `/etc/exports` Datei:
...
Fechten (unterschiedliche Zugriffsmöglichkeiten für bestimmte Clients auf dieselben Ressourcen angeben)
 a| 
Um bestimmten Clients einen unterschiedlichen Zugriff auf eine einzelne exportierte Ressource zu ermöglichen, müssen Sie jeden Client und dessen erlaubten Zugriff in auflisten `/etc/exports` Datei:
 a| 
...
Name-Aliasing
 a| 
Wenn Sie einen Export definieren, können Sie den Namen des Exports vom Namen des Dateipfads unterscheiden. Sie sollten das verwenden `-actual` Parameter beim Definieren eines solchen Exports im `/etc/exports` Datei:
 a| 
