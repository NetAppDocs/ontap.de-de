...
.Über diese Aufgabe
* Sie können Volumes nicht aus einer Konsistenzgruppe in einer SnapMirror Active Sync- oder SVM-Disaster-Recovery-Beziehung entfernen. Sie müssen zuerst die aktive Sync-Beziehung von SnapMirror unterbrechen, um die Konsistenzgruppe zu ändern und dann die Beziehung wiederherzustellen.
