...
| MetroCluster SVMs | ONTAP 9.16.1 | Sie können eine SVM von einem nicht-MetroCluster HA-Paar in eine MetroCluster Konfiguration oder von einer MetroCluster Konfiguration auf ein nicht-MetroCluster HA-Paar migrieren. Sie können eine SVM nicht von einer MetroCluster-Konfiguration zu einer anderen MetroCluster-Konfiguration migrieren. [HINWEIS] die SVM-Migration unterstützt in früheren Versionen als ONTAP 9 nicht die MetroCluster SVM-Migration.16.1. Möglicherweise können Sie die asynchrone Replikation von SnapMirror zu verwendenlink:https://www.netapp.com/media/83785-tr-4966.pdf["Migrieren einer SVM in einer MetroCluster-Konfiguration"^]. Sie sollten beachten, dass es sich bei der Verwendung von SnapMirror Asynchronous für die Migration einer SVM in einer MetroCluster-Konfiguration um eine disruptive_ Migrationsmethode handelt. 
