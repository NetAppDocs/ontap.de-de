...
IMPORTANT: Wenn Sie SnapMirror active sync und ONTAP Mediator oder ONTAP Cloud Mediator mit ONTAP 9.17.1 verwenden, sollten Sie die *Bekannten Probleme und Einschränkungen* in der link:https://library.netapp.com/ecm/ecm_download_file/ECMLP2492508["Versionshinweise zu ONTAP"^] überprüfen, um wichtige Informationen zu diesen Konfigurationen zu erhalten.
...
Wenn Sie ONTAP Mediator zur Überwachung einer SnapMirror active sync-Beziehung verwenden, kann ONTAP Mediator in ONTAP nicht ohne ein gültiges selbstsigniertes Zertifikat oder ein Zertifikat einer Zertifizierungsstelle (CA) initialisiert werden. Sie fügen dem Zertifikatsspeicher für verbundene Cluster ein gültiges Zertifikat hinzu. Wenn Sie ONTAP Mediator zur Überwachung von MetroCluster IP-Systemen verwenden, wird HTTPS nach der Erstkonfiguration nicht mehr verwendet; daher sind keine Zertifikate erforderlich.
...
-----END CERTIFICATE-----
----
. Fügen Sie das ONTAP Mediator CA-Zertifikat zu einem ONTAP Cluster hinzu. Geben Sie bei Aufforderung das vom ONTAP Mediator erhaltene CA-Zertifikat ein. Wiederholen Sie die Schritte auf allen Peer-Clustern.
+
