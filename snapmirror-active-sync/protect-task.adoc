...
.. Aktivieren Sie das Kontrollkästchen zu **SnapMirror aktivieren**, und wählen Sie dann die `AutomatedFailoverDuplex` Richtlinie:
...
--
.Schritte für eine einheitliche Konfiguration
. Erstellen einer neuen SnapMirror Beziehung, bei der alle Volumes in der Applikation gruppiert werden. Stellen Sie sicher, dass Sie den festlegen `AutomatedFailOverDuplex` Richtlinie zur bidirektionalen synchronen Replizierung.
+
...
. Bestätigen Sie, dass der Vorgang erfolgreich war, indem Sie auf warten `Mirrored State` Um als anzuzeigen `SnapMirrored` Und das `Relationship Status` Als `Insync`.
+
