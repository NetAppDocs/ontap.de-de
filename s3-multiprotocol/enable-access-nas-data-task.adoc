...
+
.. Klicken Sie auf *Speicher > Speicher-VMs*, wählen Sie eine NAS-fähige Speicher-VM aus, klicken Sie auf *Einstellungen* und dann unter *Protokolle > S3* auf image:icon_gear.gif["Aktionssymbol"] S3.
...
. S3 kann auf einer Storage-VM mit konfigurierten NAS-Protokollen aktiviert werden.
+
.. Klicken Sie auf *Cluster > Storage VMs*, wählen Sie eine NAS-fähige Storage VM aus, klicken Sie auf *Einstellungen* und anschließend unter *Protokolle > S3* auf image:icon_gear.gif["Aktionssymbol"].
.. Wählen Sie den Zertifikatstyp aus. Unabhängig davon, ob ein systemgeneriertes Zertifikat oder ein eigenes ausgewählt wird, ist es für den Clientzugriff erforderlich.
.. Die Netzwerkschnittstellen eingeben.
