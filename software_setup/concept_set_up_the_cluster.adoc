...
summary: Das Einrichten des Clusters umfasst das Erfassen der erforderlichen Informationen für die Konfiguration von Einrichtung jedes Nodes, das Erstellen des Clusters auf dem ersten Node und das Verbinden aller verbleibenden Nodes mit dem Cluster. 
---
= Sammeln von Cluster-Informationen für die Cluster-Einrichtung
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
