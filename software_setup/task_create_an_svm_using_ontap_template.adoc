...
summary: Ab ONTAP 9.3 kann eine SVM mithilfe einer vorkonfigurierten Vorlage erstellt und bereitgestellt werden. Es müssen Werte für die in der Vorlage definierten Parameter angegeben werden. 
---
= Eine SVM mithilfe einer vorkonfigurierten ONTAP Vorlage bereitstellen
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Ab ONTAP 9.3 kann eine SVM mithilfe einer vorkonfigurierten Vorlage erstellt und bereitgestellt werden. Es müssen Werte für die in der Vorlage definierten Parameter angegeben werden.

.Bevor Sie beginnen
Alle erforderlichen Funktionslizenzen müssen auf dem Cluster installiert sein.

.Über diese Aufgabe
Die VserverEnvironmentSetup Vorlage ist die vorkonfigurierte Vorlage, die für die Konfiguration von SVMs verwendet werden kann. Die VserverEnvironmentSetup Vorlage ermöglicht das Einrichten der folgenden Konfigurationen:

* Sicherheitsstil, Spracheinstellung und IPspace der SVM
* Protokolle
* Daten-LIFs
* Netzwerkrouten
* DNS-Konfiguration
...
Die VserverEnvironmentSetup Vorlage ist eine schreibgeschützte Vorlage. Die Standardparameterwerte dieser Vorlage können nicht geändert werden. Um die Standardwerte eines der Parameter in der Vorlage zu ändern, ist es erforderlich, die Vorlage zu kopieren und anschließend die Standardwerte der Parameter zu ändern.

.Schritte
. Die Werte der Parameter, die in der Vorlage zum Erstellen und Konfigurieren der SVM definiert sind, können angezeigt und notiert werden: `template parameter show`
+
[listing]
----
...
----
. Wenn der Standardwert eines Parameters in der Vorlage geändert werden soll, kann die vorhandene Vorlage angepasst werden:
+
.. Vorlage kopieren:
+
...
----
+
Die neue Vorlage ist eine lesen/schreiben-Vorlage.

.. Den Standardwert des Parameters ändern, der geändert werden soll:
+
...
. Die SVM wird bereitgestellt, indem Werte für die Parameter in der Vorlage angegeben werden:
+
